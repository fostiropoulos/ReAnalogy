...
class Xeger(RstrBase):
    ...
    def __init__(
        self,
        _random: "_Random" = typing.cast("_Random", random),
        **custom_alphabets: str,
        ...
        self._cases: Mapping[str, Callable[..., Any]] = {
            ...
            "branch": lambda x: "".join(
                self._handle_state(i) for i in self._random.choice(x[1])
            ),
    ...
    def _handle_repeat(self, start_range: int, end_range: int, value: str) -> str:
        ...
        times = self._random.randint(max(start_range, 1), max(end_range, 2))
...
if __name__ == "__main__":
    ...
    p = Xeger().xeger("https://www.python.org")
    p = Xeger().xeger("\\n")
